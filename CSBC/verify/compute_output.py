...
from verify.interval_number import IntervalNumber, interval_max, inf, sup, interval_sigmoid, interval_tanh, interval_cos_sin
from parameters import system_dim
from verify.verify_utils import mycallback
from parameters import a_bound
...
def milp_output_area_actor(x_min, x_max, W, b):  # -1: init, 0: derivative, 1: unsafe
    weight_size = len(W)
    input_size = len(W[0])
    intval_x = []
    for i in range(0, input_size):
    ...
    for t_layer_index in range(1, t_num_layers):
        ...
        for tj in range(0, len(layer_outputs[t_layer_index - 1][0])):
            if inf(layer_outputs[t_layer_index-1][0][tj]) >= 0:
                # active_flag=1 means the interval value of this neuron do not include negative number,
                # means ReLU activation function does not work
                # y=ReLU(w1*x+b1) ==> y=w1*x+b1
                # the next layer z=ReLU(w2*y+b2) ==> z=ReLU(w2*(w1*x+b1)+b2) ==> z=ReLU(w2*w1*x+w2*b1+b2)
                active_flag[tj] = 1
        ...
        for i in range(0, len(active_flag)):
            if active_flag[i] == 1:
                ...
                W_active.append(W[t_layer_index][i, :])  # w2
                W_mixed.append(W[t_layer_index-1][:, i])  # w2*w1*x
                b_mixed.append(b[t_layer_index-1][i])  # w2*b1
            else:
