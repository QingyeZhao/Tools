...
def parameter_load_controller(load_net_dir, num_hidden_layers):
...
def get_parameters(load_net_dir):
    ...
    controller_parameters = parameter_load_controller(load_net_dir, len(controller_structure) - 2)
    controller_w = []
    controller_b = []
...
def mycallback(model, where):
    if where == GRB.Callback.MIP:
        ...
        if abs(objbst - objbnd) < 0.99 * abs(objbst):
            # print('Stop early - 99% gap achieved')
            model.terminate()
