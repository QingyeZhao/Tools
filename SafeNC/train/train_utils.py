...
def get_data():
    ...
    return data_unsafe, data_init, data_inv


def save_parameters(sess, barrier_saving_parameters, barrier_layer_neurons, barrier_type,
    ...
    if not os.path.exists(save_net_dir + "controller/"):
        os.mkdir(save_net_dir + "controller/")
    ...
    for i in range(0, len(controller_saving_parameters)):
        p = sess.run(controller_saving_parameters[i])
        if i % 2 == 0:
            np.savetxt(save_net_dir + "controller/w" + str(int(i / 2) + 1), p)
        else:
            np.savetxt(save_net_dir + "controller/b" + str(int(i / 2) + 1), p)
    ...
    with open(save_net_dir + "controller_structure", "w") as f:
        for i in range(0, len(controller_layer_neurons)):
            f.write(str(controller_layer_neurons[i]) + "\n")

    with open(save_net_dir + "barrier_type", "w") as f:
        f.write(str(barrier_type))

    with open(save_net_dir + "controller_type", "w") as f:
